...
const platformCommands: Record<Platform, Record<InstallMode, string>> = {
  ...
  windows: {
    ...
    quick:
      'irm "https://quickpaper.nicompter.de/install?os=win" | iex; # Run with: -NonInteractive -Yes -AcceptEula -AutoUpdate',
    custom:
      'irm "https://quickpaper.nicompter.de/install?os=win" -OutFile install.ps1; .\\install.ps1 -MCVersion 1.21.4 -MinRam 2G -MaxRam 4G -Port 25565 -AcceptEula -AutoUpdate',
  },
};
...
export function InstallationSection({ t }: InstallationSectionProps) {
  ...
  const modes: {
    ...
    data: { title: string; description: string; command: string };
  }[] = [
    {
      key: "interactive",
      icon: MessageSquare,
      data: {
        ...
        command: platformCommands[activePlatform].interactive,
      },
    },
    {
      key: "quick",
      icon: Rocket,
      data: {
        ...t.installation.quick,
        command: platformCommands[activePlatform].quick,
      },
    },
    {
      key: "custom",
      icon: Settings,
      data: {
        ...t.installation.custom,
        command: platformCommands[activePlatform].custom,
      },
    },
  ];
...
function TerminalDemo({
  ...
  const serverDir = isWindows
    ? "C:\\Users\\User\\paper-server"
    : "~/paper-server";
 ...
 | |__| | |_| | | (__|   <| |_) | (_| | |_) |  __/ |      
  \\___\\_\\\\__,_|_|\\___|_|\\_\\ .__/ \\__,_| .__/ \\___|_|      
        ...
        <div className="text-zinc-500 text-xs mt-1 mb-4">
          Minecraft Paper Server Installer {isWindows ? "(Windows)" : ""}
        </div>
        ...
        <div className="space-y-2 text-zinc-300">
          <div className="flex items-center gap-2">
            ...
            <span>
              {isDE ? "Prüfe Abhängigkeiten..." : "Checking dependencies..."}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-cyan-400">→</span>
            <span>
              {isDE ? "Lade Build-Informationen..." : "Fetching build info..."}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-cyan-400">→</span>
            <span>
              {isDE ? "Lade Paper herunter..." : "Downloading Paper..."}
            </span>
          </div>
        </div>
        ...
        <div className="mt-6 p-4 bg-green-500/10 border border-green-500/30 rounded-lg">
          <div className="flex items-center gap-2 text-green-400">
            ...
            <span className="font-bold">
              {isDE ? "Installation abgeschlossen!" : "Installation complete!"}
            </span>
          </div>
          <div className="text-zinc-400 mt-2 text-xs space-y-1">
            <div>
              <span className="text-zinc-500 font-bold">Server directory:</span>{" "}
              {serverDir}
            </div>
            <div>
              <span className="text-zinc-500 font-bold">Start server:</span>{" "}
              {startCmd}
            </div>
            <div>
              <span className="text-zinc-500 font-bold">Port:</span> 25565
            </div>
            <div>
              <span className="text-zinc-500 font-bold">Minecraft:</span> 1.21.4
              (build 232)
            </div>
          </div>
        </div>
